...
@onready var t_button = $HUD/Button as Button
signal kill_me

func _ready():
	t_button.button_down.connect(_kill_me)
...
func place_tutorial_nodes():
	
	await get_tree().create_timer(1.0).timeout
	place_custom($"1".position)
	await get_tree().create_timer(1.0).timeout
	place_custom($"2".position)
	await get_tree().create_timer(1.0).timeout
	place_custom($"3".position)
	...
	for node in get_tree().get_nodes_in_group("mycelia_nodes"):
		node.queue_free()
	for node in get_tree().get_nodes_in_group("connectors"):
		node.queue_free()
	for node in get_tree().get_nodes_in_group("danger"):
		node.queue_free()
...
func place_custom(pos):
	if check_node_collision(pos)[1].is_empty():
			...
			var node = add_mycelia_node(pos)
			select_play_audio()
			var collisions = expanding_collision() #do something with this
			node.set_connection_list( add_connections(node, custom_collision(20, pos )[1]))
...
func add_danger_node(pos_danger):
	...
	node_danger.add_to_group("danger")
